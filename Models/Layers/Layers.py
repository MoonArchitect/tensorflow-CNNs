...
class ShakeDrop(nn.layers.Layer):
    ...
    """ @tf.function """
    @tf.custom_gradient
    def _ShakeDrop(self, inputs):
        b = tf.cond(
            tf.random.uniform([1], 0, 1)[0] <= self.p,
        ...
        # return input, grad
        return tf.scalar_mul(b, inputs), grad
...
def decay_fn(pos_val_pairs, exponent = 1):
    ...
    assert all([pos_val_pairs[0][i] > pos_val_pairs[0][i - 1] for i in range(1, len(pos_val_pairs[0]))]), \

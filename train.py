...
# lr_schedule	code		LearningRate.HTD(-6, 3, 50, 0.15, 0.0002, 5)
# batch size	int			64
# epochs		int			300
# data-dir
...
def train(
	...
	callbacks = [
		...
		tf.keras.callbacks.ModelCheckpoint(
			f"Checkpoints\\{name}\\Best\\" + name + r"_{val_accuracy:3f}%",			# TODO check with / instead of \\
...
def main():		# TODO when called creates new name (date) even if id is already in checkpoint 
	...
	if args.checkpoint != None and not ".yaml" in args.checkpoint:
	...
	model = utils.registry.create_model(model_name, **model_kwargs)
	optimizer = utils.registry.create_optimzer(optimizer_name, **optimizer_kwargs)
	lr_schedule = utils.registry.create_lr_schedule(lr_schedule_name, **lr_schedule_kwargs)
